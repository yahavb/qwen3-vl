"""Qwen3-VL-8B TP-8 — fused submodule compilation benchmark.

Same structure as serve_tp8.py but uses model_fused.py + loader_fused.py
which compile entire AttentionBlock and MLPBlock submodules instead of
individual matmuls. This lets the Neuron compiler fuse:
  - norm + QKV projection + QK-norm + RoPE into one NEFF
  - norm + gate_up + SiLU + down into one NEFF

Compare tok/s against serve_tp8.py (baseline) to measure improvement.

Usage:
    torchrun --nproc_per_node=8 serve_tp8_fused.py
"""
...
if rank == 0:
    print("=" * 60)
    print(f"  Qwen3-VL-8B TP-8 — FUSED Submodule Compilation")
    print(f"  Compiling whole AttentionBlock + MLPBlock (fewer HBM trips)")
    print(f"  World size: {world_size}")
    print(f"  MAX_NFRAMES: {MAX_NFRAMES}, MAX_NEW_TOKENS: {MAX_NEW_TOKENS_DEFAULT}")
    print("=" * 60)

# ═══════════════════════════════════════════════════════════════════════
# Load model (fused version)
# ═══════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════
# WARMUP: Text-only — triggers NEFF compilation for prefill + decode
# ═══════════════════════════════════════════════════════════════════════
if rank == 0:
    print("\n" + "=" * 60)
...
# Longer generation for throughput measurement
if rank == 0:
...
if rank == 0:
    print("\n[DONE] Fused profiling complete. Exiting.")
import os as _os
_os._exit(0)
